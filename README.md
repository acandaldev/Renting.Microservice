...
2. **Azure Container Registry (ACR)** - To host Docker images.
3. **Azure Kubernetes Service (AKS)** - To orchestrate the microservice.
4. **Azure Cosmos DB (MongoDB API)** - Serverless NoSQL database.
5. **Azure Key Vault** - Securely stores the Cosmos DB credentials automatically generated by Terraform.
...
A fully automated CI/CD pipeline using **GitHub Actions** is defined in .github/workflows/ci-cd.yml.

### The Secure Deployment Flow (No secrets in GitHub)

1. **Build & Test**: Restores, builds, and runs all xUnit tests.
2. **Docker Build & Push**: Builds the image tagged with the Git SHA and pushes to ACR.
3. **Deploy to AKS**:
   - Replaces the __KEY_VAULT_NAME__ placeholder in deployment.yaml with the public GitHub Variable.
